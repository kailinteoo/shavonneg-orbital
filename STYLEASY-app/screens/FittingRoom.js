...
const FittingRoom = () => {
      ...
      try {
        ...
        if (currentUser) {
          ...
          userImagesSnapshot.forEach((doc) => {
            shoesImagesData.push({ id: doc.id, url: doc.data().url });
          });
  ...
  const handleGenerateRandomImage = () => {
    const randomIndex = Math.floor(Math.random() * shoesImages.length);
    const randomImageUrl = shoesImages[randomIndex]?.url;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.imageContainer}>
        {randomImage ? (
          <View style={styles.randomImageWrapper}>
            <Image
